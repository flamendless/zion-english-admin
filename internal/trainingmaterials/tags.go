...
func GetOrCreateTag(ctx context.Context, q *queries.Queries, label string, colorIndex int) (int64, error) {
	existing, err := q.GetTrainingMaterialTagByLabel(ctx, label)
	if err == nil {
	...
	id, err := q.InsertTrainingMaterialTag(ctx, queries.InsertTrainingMaterialTagParams{
		Label: label,
...
func ReplaceMaterialTags(ctx context.Context, q *queries.Queries, materialID int64, labels []string) error {
	...
	if err := q.DeleteTrainingMaterialTagLinks(ctx, materialID); err != nil {
		return fmt.Errorf("clear tag links: %w", err)
	}

	tagCount, err := q.CountTrainingMaterialTags(ctx)
	if err != nil {
	...
	for i, label := range normalized {
		...
		if err := q.InsertTrainingMaterialTagLink(ctx, queries.InsertTrainingMaterialTagLinkParams{
