...
usage() {
	cat <<'USAGE'
Usage: ./scripts/dbbackup.sh [OPTIONS]

Options:
  -a, --app-dir <DIR>     Zion English app directory
                          Default: script parent directory, or APP_DIR
  -u, --db-url <URL>      SQLite DB URL/path
                          Default: GOOSE_DBSTRING from env/.env, or file:./data/zion.db
...
Examples:
  ./scripts/dbbackup.sh                                      # Backup DB to object storage
  ./scripts/dbbackup.sh --local                              # Backup DB to ~/backups/zion-english
  ./scripts/dbbackup.sh --local /path/to/backups             # Backup DB to a custom local directory
  ./scripts/dbbackup.sh --db-url file:/data/zion.db          # Backup a specific SQLite DB
USAGE
	exit 0
}
...
load_db_url_from_env_file() {
	...
	db_line="$(grep -E '^[[:space:]]*GOOSE_DBSTRING=' "$env_file" | tail -n 1 || true)"
	if [[ -z "$db_line" ]]; then
...
BACKUP_NAME="zion_english_backup_${TS}"
TMP_DIR="$(mktemp -d)"
ARCHIVE_ROOT="$TMP_DIR/$BACKUP_NAME"
ARCHIVE_PATH="$TMP_DIR/${BACKUP_NAME}.tar.gz"
SQLITE_BACKUP="$ARCHIVE_ROOT/zion.sqlite"
