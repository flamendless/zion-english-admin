...
func handleProfile(w http.ResponseWriter, r *http.Request) {
	...
	if introVideo, err := dbRO.GetQueries().GetLatestTeacherIntroVideoByTeacherID(ctx, user.ID); err == nil {
		...
		data.IntroVideoViewURL = introVideoViewURL(introVideo.ID, introVideo.Url, introVideo.Status)
...
func handleProfileAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if err := store.Put(ctx, storage.CategoryAvatars, filename, file, avatarContentType(filename)); err != nil {
		...
		insertUploadLog(ctx, user, uploadLogEntry{
			...
			Kind:          constants.UploadLogKindAvatar,
			Summary:       fmt.Sprintf("profile picture storage failed for teacher '%s' (id %d), file '%s': %v", user.Name, user.ID, filepath.Base(header.Filename), err),
	...
	if err := dbRW.GetQueries().UpdateTeacherProfilePicture(ctx, queries.UpdateTeacherProfilePictureParams{
		...
		insertUploadLog(ctx, user, uploadLogEntry{
			...
			Outcome:       constants.UploadLogOutcomeFailed,
			Kind:          constants.UploadLogKindAvatar,
			Summary:       fmt.Sprintf("profile picture database update failed for teacher '%s' (id %d), file '%s': %v", user.Name, user.ID, filepath.Base(header.Filename), err),
			Filename:      filepath.Base(header.Filename),
			FileSize:      header.Size,
			FileSizeValid: true,
		})
	...
	insertUploadLog(ctx, user, uploadLogEntry{
		...
		Kind:          constants.UploadLogKindAvatar,
		Summary:       fmt.Sprintf("profile picture for teacher '%s' (id %d), file '%s'", user.Name, user.ID, filepath.Base(header.Filename)),
		Filename:      filepath.Base(header.Filename),
		FileSize:      header.Size,
		FileSizeValid: true,
	})
